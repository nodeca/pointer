 ...
 *  - handler (Function): Handler ssociated with Route and returned as part
 *    of MatchData by [[Route#match]] on successfull matching.
 *
 *  Route constructor.
 *
 *
 ...
 *  - *handler* (Function): Handler associated with route.
 **/
Route.prototype.match = function match(url) {
  var data, captures = String(url).match(this.__regexp__);

  if (captures) {
    data = {route: this, params: {}, handler: this.__handler__};
...
Route.prototype.isValidParams = function isValidParams(params) {
  ...
  // nd that they match associated regexp matchers
  Common.each(this.__params__, function (cfg, key) {
    if ('undefined' === typeof params[key] || !cfg.match_re.test(params[key])) {
      is_valid = false;
      return false;
    }
  });
