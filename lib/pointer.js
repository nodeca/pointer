...
Pointer.prototype.addRoute = function addRoute(pattern, options) {
  ...
  if (options.name) {
    ...
    if ('[object Array]' !== Object.prototype.toString.call(nameList)) {
      nameList = [ nameList ];
    }

    for (nameIndex = 0, nameAmount = nameList.length;
         nameIndex < nameAmount;
         nameIndex += 1) {
...
/**
 *  Pointer#linkTo(name[, params[, prefixOptions]]) -> String|Null
 ...
 *  - [[Route#buildURL]]
 **/
Pointer.prototype.linkTo = function linkTo(name, params, prefixOptions) {
  return find(this.__named_routes__[name] || [], function (route) {
    return route.buildURL(params, prefixOptions);
  }) || null;
};
