...
function promote(parent, key) {
  ...
  for (i in parent[key]) {
    if ({}.hasOwnProperty.call(key, i)) t[i] = parent[key][i];
  }
...
function set(obj, key, val) {
  // base could be a string (e.g. `?[]=foo`),
  // so we need to check if it's an object
  if (!(typeof obj === 'object' && obj !== null)) return;


  var v = obj[key];

  if (typeof v === 'undefined') obj[key] = val;
  else if (Array.isArray(v)) v.push(val);
  else obj[key] = [ v, val ];
}
  ...
  if (key.indexOf(']') >= 0) {
    ...
    if (!isint.test(key) && Array.isArray(parent.base)) {
      ...
      for (k in parent.base) {
        if ({}.hasOwnProperty.call(parent.base, k)) t[k] = parent.base[k];
      }
