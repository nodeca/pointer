...
function Route(pattern, params, meta, prefix, parseURL) {
  ...
  if (prefix) {
    ...
    // change here.
    if (this.__prefix_pathname__ === '/' && this.__prefix__.slice(-1) !== '/') {
      this.__prefix_pathname__ = '';
    }
  }

  // alias '' to '/' if there is no path in the prefix,
  // example: { pattern: '', prefix: 'http://example.com' } -> http://example.com/
  if (pattern === '' && !this.__prefix_pathname__) {
    pattern = '/';
  }
