...
function Route(pattern, params, meta, prefix) {
  ...
  if (this.__prefix__) {
    var parsed = URL(this.__prefix__);

    this.__prefix_protocol__ = parsed.attr('protocol');
    this.__prefix_hostname__ = parsed.attr('host');
    this.__prefix_port__     = parsed.attr('port');
    this.__prefix_pathname__ = parsed.attr('path');
  }
}
...
/**
 *  Route#buildURL(params[, prefixOptions]) -> String|Null
 *  - params (Object): Params to fill into resulting URL.
 *  - prefixOptions (Object): URL parts to compose URL prefix: protocol,
 *    hostname, port, and pathname. Each will be used if prefix has no own.
 *
 *  Returns URL representation of the route with given params.
 *  Returns `Null` when there were not enough params to build URL.
 **/
Route.prototype.buildURL = function buildURL(params, prefixOptions) {
  prefixOptions = prefixOptions || {};

  var prefix   = '',
      url      = this.__builder__.build(params),
      protocol = this.__prefix_protocol__ || prefixOptions.protocol,
      hostname = this.__prefix_hostname__ || prefixOptions.hostname,
      port     = this.__prefix_port__     || prefixOptions.port,
      pathname = this.__prefix_pathname__ || prefixOptions.pathname;

  if (!url) {
    return null; // Can't build.
  }
